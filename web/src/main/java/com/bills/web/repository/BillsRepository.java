...
public interface BillsRepository extends CrudRepository<Bills,Long>{
    ...
    @Query(value = "SELECT * FROM bills WHERE ((EXTRACT(MONTH FROM date_bills) =:month AND EXTRACT(YEAR FROM date_bills) =:year) or (type ='fixed' and EXTRACT(MONTH FROM date_bills) <=:month AND EXTRACT(YEAR FROM date_bills) <=:year))  AND user_id=:user", nativeQuery=true)
